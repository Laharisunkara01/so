...
heroI=loadImage("sprites/jaill.png");
kI=loadImage("sprites/kk.png");
oI=loadImage("sprites/over.png");
...
function draw() {
  ...
  if(gameState==="play"){
    ...
    if(score>=2){
      ...
      hero=createSprite(1100,100,20,20);
      hero.scale=0.3;
      hero.addImage(heroI)

     if(keyWentDown('k')){
      k=createSprite(player.x,player.y,10,10);
      k.addImage(kI);
      k.scale=0.2;
      k.lifetime=50;
      kGroup.add(k)
  ...
  if(gameState==="end"){
    ...
    o=createSprite(530,170,20,20);
    o.addImage(oI);

    textSize(25)
    fill("black")
    text("Press R to retart the game",400,270)
    console.log("working")
  }
...
function spawnObstacle()
  ...
  function firePit(){
    if(frameCount % 130===0){
      ...
      f=createSprite(1200,320,20,20);
      f.addAnimation("firing",fI);
      f.scale=0.3;
